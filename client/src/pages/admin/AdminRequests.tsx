...
const AdminRequests: React.FC = () => {
  ...
  useEffect(() => {
    load();
    const channel = subscribeToItineraryRequests(() => {
      load();
    });
    return () => {
      unsubscribeAll();
  ...
  const saveNotes = async () => {
    const selId = (selected as any)?.id || (selected as any)?._id;
    if (!selId) return;
    setSaving(true);
    setError("");
    try {
      await updateItineraryRequest(selId, { adminNotes: notes });
      clearCache("itinerary-requests");
      setSelected(null);
      load();
    } catch (err: any) {
            ...
            >
              ...
              <tbody>
                {filteredItems.map((req) => (
                  <tr
                    key={(req as any).id || (req as any)._id}
                    style={{
                    ...
                    >
                      ...
                      {(req.status || "pending") === "pending" && (
                        <Button
                          ...
                          onClick={() => {
                            const id = (req as any).id || (req as any)._id;
                            id && updateStatus(id, "in-progress");
                          }}
                          disabled={saving}
                        >
                          ▶ Start
              ...
              <Row className="g-3">
                <Col xs={12} sm={6}>
                  ...
                  <Form.Select
                    ...
                    onChange={(e) => {
                      const id = (selected as any).id || (selected as any)._id;
                      id && updateStatus(id, e.target.value);
                    }}
                    disabled={saving}
                    style={{
                      background: darkMode ? "#1e1e2e" : "#fff",
